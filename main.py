...
def computeCost(x_train1, x_train2, y_origin, b00, b11, b22): # loss funtion
    ...
    for i in range(len(y_origin)):
        sum += ((b00 + b11 * x_train1[i] / 10000 + b22 * x_train2[i] / 10) - y_origin[i])**2
...
def gradientDescent(x_train1, x_train2, y_origin, b0, b1, b2, learning_rate):
    db0, db1, db2 = 0, 0, 0
    for i in range(len(y_origin)):
        # db0 += (b0 + b1 * (x_train1[i] - np.mean(x_train1[i])) / np.std(x_train1[i]) + b2 * (x_train2[i] - np.mean(x_train2[i])) / np.std(x_train2[i]) - y_origin[i])
        # db1 += (b0 + b1 * (x_train1[i] - np.mean(x_train1[i])) / np.std(x_train1[i]) + b2 * (x_train2[i] - np.mean(x_train2[i])) / np.std(x_train2[i]) - y_origin[i]) * x_train1[i]
        # db2 += (b0 + b1 * (x_train1[i] - np.mean(x_train1[i])) / np.std(x_train1[i]) + b2 * (x_train2[i] - np.mean(x_train2[i])) / np.std(x_train2[i]) - y_origin[i]) * x_train2[i]
        db0 += (b0 + b1 * x_train1[i] / 10000 + b2 * x_train2[i] / 10 - y_origin[i])
        db1 += (b0 + b1 * x_train1[i] / 10000 + b2 * x_train2[i] / 10 - y_origin[i]) * x_train1[i]
        db2 += (b0 + b1 * x_train1[i] / 10000 + b2 * x_train2[i] / 10 - y_origin[i]) * x_train2[i]
...
if __name__ == '__main__':
    ...
    # X, Y = np.meshgrid(dien_tich, sl_phong_ngu)
